"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.CatalogSchema = exports.Catalog = void 0;
const mongoose_1 = require("@nestjs/mongoose");
let Catalog = class Catalog {
};
exports.Catalog = Catalog;
__decorate([
    (0, mongoose_1.Prop)({ required: true }),
    __metadata("design:type", String)
], Catalog.prototype, "name", void 0);
__decorate([
    (0, mongoose_1.Prop)({ required: true }),
    __metadata("design:type", Boolean)
], Catalog.prototype, "primary", void 0);
__decorate([
    (0, mongoose_1.Prop)({ default: () => new Date().toISOString() }),
    __metadata("design:type", String)
], Catalog.prototype, "indexedAt", void 0);
exports.Catalog = Catalog = __decorate([
    (0, mongoose_1.Schema)()
], Catalog);
exports.CatalogSchema = mongoose_1.SchemaFactory.createForClass(Catalog);
exports.CatalogSchema.virtual('id').get(function () {
    return this._id.toHexString();
});
exports.CatalogSchema.set('toJSON', { virtuals: true });
exports.CatalogSchema.set('toObject', { virtuals: true });
//# sourceMappingURL=catalog.schema.js.map